...
async def permalink_heno(c, msg):
    ...
    if msg.reply_to_message.from_user.id == 7157276873:
        return await c.send_message(msg.chat.id, f"**- دا ماندو صاحب السورس عيب**")
    if msg.reply_to_message.from_user.id == 6906453300:
    ...
    animation_chars = [                "**🝳︙ جـاري الاتصـال بجهـاز الضحـية لأختـراقـة  📳**",     
       ...
       "**🝳︙ تحـميل الاخـتراق  ㊙️ .. 36%**\n█████████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒ `",        
        ...
        "**🝳︙ تحـميل الاخـتراق  ㊙️ .. 100%**\n████████████████████████`",       
         "**🝳︙ تـم اخـتراق الضحـية 🆘 بواسطه : `{الههكر ماندو}` . بـدون تنـازل قم بدفع مبلغ والا سيقوم بنشر صورك**",
            ]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
...
async def tikol_1(c, msg):
    ...
    dev_ids = [7157276873, 7157276873, 7157276873]  

    
    if msg.reply_to_message.from_user.id in dev_ids:
        await msg.reply("**لا ده المطور يكسمك .**")
        return
