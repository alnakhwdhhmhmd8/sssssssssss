...
async def msong(app, msg):
    ...
    if "https://www.instagram.com/" in link :
        ...
        if rr["status"] == "Done":
            ...
            copy=f"""↯ | Dev⇾ [𝐆 𝐀 𝐊 - مــانــدو](t.me/wvvwv3)"""
            try:
...
def insts(link):
    ...
    if res["status"] == "ok":
        ...
        return {"status":"Done","vedio":vedio,"POWEREDBY":"@wvvwv3"}
    else:
        return {"status":"none","POWEREDBY":"@wvvwv3"}
...
def social(url):
	try:
		...
		if not dl.get("message"):
			return {"status":"Done", "vedio": dl["result"]["url"], "POWEREDBY":"@wvvwv3"}
		else:
			return {"status":"none", "POWEREDBY":"@wvvwv3"}
