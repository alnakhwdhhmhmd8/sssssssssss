...
async def hamohossam(client, message):
    cae_s_ar = 0
    hos1 = 0
    hos2 = 0
...
async def leave_group(c,msg):
  await msg.edit("◐ يتم مغادرة المجموعه ....🚶‍♂️")
  await asyncio.sleep(.5)
  await msg.edit("◐ تم مغادرة المجموعه بنجاح 👨‍✈️ .")
...
async def tgph(c, msg):
    ...
    await msg.reply_to_message.download("./M7_N4")
    await msg.edit("جاري رفع الصورة علي تلجراف")
    response = upload_file("./M7_N4")
    remove("./M7_N4")
...
async def ceev(client, message):
    if message.reply_to_message.from_user.id == 1861200374:
        await message.reply_text("◐ عذرآ لا تستطيع استخدام الأمر على المبرمج ماندو👨‍✈️ .")
    else:
        hamo = await client.get_chat(message.from_user.id)
        RIVO = hamo.username
